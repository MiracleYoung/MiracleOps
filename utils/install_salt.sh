...
if [ ${_NO_DEPS} -eq $BS_FALSE ] && [ $_CONFIG_ONLY -eq $BS_FALSE ]; then
    # Only exec function is not in config mode only
...
# Run any post install function. Only exec function if not in config mode only
...
# Run any check services function, Only exec function if not in config mode only
